...
def getGradientPoints(image):
    ...
    cv2.namedWindow("gr", cv2.CV_WINDOW_AUTOSIZE)
    cv2.imshow("gr",grad_approx)
    cv2.waitKey(0) & 0xFF
    cv2.destroyWindow("gr")
    averageValue = numpyLib.average(grad_approx)
    nonZeroPoints = numpyLib.nonzero(grad_approx > averageValue * 0.5)
...
def getPointsFromImage(fileName, size_of_ann):
    ...
    cv2.namedWindow("temp", cv2.CV_WINDOW_AUTOSIZE)
    ...
    #img2 = cv2.drawKeypoints(img, mergedPoints, color=(255, 0, 0), flags=0)
...
net = neurolabLib.net.newff(boundaries, [10, 1])
# Train process
